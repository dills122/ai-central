...
for selected_profile in $profile; do
  case "$selected_profile" in
    base|javascript-typescript|angular|dotnet-csharp|dotnet-clean-architecture|dotnet-aspnetcore|dotnet-efcore|dotnet-orleans|dotnet-aspire|dotnet-opentelemetry|dotnet-grpc|kotlin-jvm|rust|shell-scripting|payload|frontend-design|infrastructure-opentofu) ;;
    *)
...
if profile_selected dotnet-csharp || profile_selected dotnet-clean-architecture || profile_selected dotnet-aspnetcore || profile_selected dotnet-efcore || profile_selected dotnet-orleans || profile_selected dotnet-aspire || profile_selected dotnet-opentelemetry || profile_selected dotnet-grpc; then
  install_reusable_if_missing "$repo_root/templates/steering/dotnet-csharp-steering.md" "$target_dir/.codex/steering/dotnet-csharp-steering.md"
fi

for dotnet_profile in clean-architecture aspnetcore efcore orleans aspire opentelemetry grpc; do
  if profile_selected "dotnet-$dotnet_profile"; then
    install_reusable_if_missing "$repo_root/templates/steering/dotnet-$dotnet_profile-steering.md" "$target_dir/.codex/steering/dotnet-$dotnet_profile-steering.md"
  fi
done
